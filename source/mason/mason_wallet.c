...
bool mason_read_rsa_keypair(uint8_t *key)
{
    ...
    memcpy(key, key_pair, MAX_RSA_STORAGE_SIZE);
    if (ERT_OK != mason_storage_read(key_pair, MAX_RSA_STORAGE_SIZE, FLASH_ADDR_RSA_KEYPAIR_D))
    {
        return false;
    }
    memcpy(key + MAX_RSA_STORAGE_SIZE, key_pair, MAX_RSA_STORAGE_SIZE);
    if (ERT_OK != mason_storage_read(key_pair, MAX_RSA_STORAGE_SIZE, FLASH_ADDR_RSA_KEYPAIR_N))
    {
        return false;
    }
...
bool mason_change_wallet_passphrase(uint8_t *passphrase, uint16_t passphrase_len)
{
    ...
    // clear the ADA ROOT Key once change passhrase;
    uint8_t empty_key[MAX_RSA_STORAGE_SIZE] = {0};
    is_succeed = mason_storage_write_buffer(empty_key, MAX_RSA_STORAGE_SIZE, FLASH_ADDR_ADA_ROOT_KEY);
    if (!is_succeed)
    {
        return false;
    }
...
bool mason_delete_wallet(void)
{
    ...
    // clear user wallet related data
    uint32_t clean_address_list[7] = {FLASH_ADDR_WALLET_START, FLASH_ADDR_SEED_FROM_ENTROPY, FLSAH_ADDR_RSA_KEYPAIR_P_Q,
                                      FLSAH_ADDR_RSA_KEYPAIR_P_Q, FLASH_ADDR_RSA_KEYPAIR_D, FLASH_ADDR_RSA_KEYPAIR_N,
                                      FLASH_ADDR_ADA_ROOT_KEY};

    for (size_t i = 0; i < 7; i++)
    {
        is_succeed = mason_storage_write_buffer(key_pair, MAX_RSA_STORAGE_SIZE, clean_address_list[i]);
        if (!is_succeed)
        {
            return false;
        }
